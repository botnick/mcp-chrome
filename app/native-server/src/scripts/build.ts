...
This application is automatically started by the Chrome extension; no manual execution is required.
`;
...
filesToMakeExecutable.forEach((file) => {
  ...
  try {
    if (fs.existsSync(filePath)) {
      ...
      console.log(`Set executable permissions (755) for ${file}`);
    } else {
      console.warn(`Warning: ${filePath} does not exist, cannot set executable permissions`);
    }
  } catch (error) {
