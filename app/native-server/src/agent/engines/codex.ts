 ...
 *
 * Based on the Codex adapter event protocol from other/cweb, this engine fully handles
 * item.started/item.delta/item.completed/item.failed/error events, and pushes encoded
 * RealtimeEvents to the sidepanel via AgentStreamManager, ensuring the full data loop:
 * Sidepanel -> Native Server -> Codex CLI -> Sidepanel.
 */
