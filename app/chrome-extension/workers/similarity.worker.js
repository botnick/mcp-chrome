...
let modelInputNames = null; // Stores model input names
...
// Configure ONNX Runtime environment (once)
function configureOrtEnv(numThreads = 1, executionProviders = ['wasm']) {
  if (ortEnvConfigured) return;
  try {
    ort.env.wasm.numThreads = numThreads;
    ort.env.wasm.simd = true; // Enable SIMD when possible
    ort.env.wasm.proxy = false; // In Worker, proxy is usually not needed
    ...
    sessionOptions = {
      ...
      // executionMode: 'sequential' // Within worker, tasks are usually executed sequentially
    };
  } catch (error) {
    console.error('Worker: Failed to configure ORT environment', error);
    throw error; // Rethrow to notify the main thread
  }
}
...
async function initializeModel(modelPathOrData, numThreads, executionProviders) {
  try {
    ...
    if (modelPathOrData instanceof ArrayBuffer) {
      ...
      modelPathInternal = modelPathOrData; // Store model path for debugging or reload if needed
    ...
    session = null; // Clean up session in case of partial initialization
    modelInputNames = null;
    // Serialize error info since Error objects may not be directly postMessage-able
...
async function runBatchInference(batchData) {
  ...
  try {
    ...
    // Batch fill data (avoid map operations)
    ...
    // Handle token_type_ids - only provide when the model requires it
...
self.onmessage = async (event) => {
  ...
  try {
    ...
    // Send error as a plain object since Error objects may not serialize correctly
    self.postMessage({
      id,
