    ...
    _formatStepText(step, _idx) {
      try {
        ...
        if (t === 'click' || t === 'dblclick') {
          return `${t === 'dblclick' ? 'dblclick' : 'click'}: ${sel || '(document)'}`;
        }
        if (t === 'fill') {
        ...
        if (t === 'openTab') return `openTab: ${step.url || ''}`;
        if (t === 'switchTab') return `switchTab: contains ${step.urlContains || ''}`;
        if (t === 'switchFrame')
          return `switchFrame: contains ${step.frame && step.frame.urlContains ? step.frame.urlContains : ''}`;
        if (t === 'waitFor') return `waitFor: ${sel || step.until || ''}`;
        return `${t}`;
      } catch (_) {
        return 'step';
      }
