...
export default defineConfig({
  ...
  runner: {
    ...
    // Option 2: To enable auto-launch with existing profile, uncomment below
    // chromiumArgs: [
  ...
  manifest: {
    ...
    // Note: The following security policies block dev server resource loading,
    // so they are only enabled in production; dev mode uses WXT default policies.
    ...(IS_DEV
      ? {}
      : {
  ...
  vite: (env) => ({
    ...
    build: {
      // Build output must be compatible with ES6
      target: 'es2015',
      // Generate sourcemaps in non-production environments
      sourcemap: env.mode !== 'production',
      // Disable gzip compressed size reporting as compressing large files can be slow
