...
export function useBuilderStore(initial?: FlowV2 | null) {
  ...
  function setNodePosition(id: string, x: number, y: number) {
    ...
    // Not recorded in history stack to avoid frequent entries; user actions (connect/add/delete) trigger recording.
  }
  ...
  function onConnect(sourceId: string, targetId: string, label: string = 'default') {
    ...
    // Single same-label outgoing edge: remove existing edges with same source + same label
    for (let i = edges.length - 1; i >= 0; i--) {
      const e = edges[i];
  ...
  // Fallback layout: layered + barycenter ordering (no external dependencies)
  ...
  // - Dynamically imports elkjs to avoid bundling overhead
