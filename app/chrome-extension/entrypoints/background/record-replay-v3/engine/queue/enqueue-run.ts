 ...
 *
 * Design rationale:
 * - Extracts enqueue logic from RpcServer into an independent service
 * - Prevents behavioral drift between RPC and TriggerManager
 * - Unifies parameter validation, Run creation, queue enqueue, and event publishing
 */
...
export interface EnqueueRunDeps {
  /** Storage layer (only needs flows/runs/queue) */
 ...
 * @description Sorted by schedule order: priority DESC + createdAt ASC
...
export async function enqueueRun(
  ...
  // 4. Compute queue position (before kick, to reduce race conditions leading to position=-1)
