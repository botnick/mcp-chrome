 ...
 * @description
 * Uses chrome.alarms when parameter to implement one-time scheduled triggers.
 *
 * Behavior:
 * - Each trigger maps to a one-time alarm
 * - Automatically disables (enabled=false) and uninstalls after firing
 */
...
export interface OnceTriggerHandlerDeps {
  ...
  /**
   * Optional: custom method to disable triggers
...
export function createOnceTriggerHandler(
  ...
  // Lazy-create store to avoid issues in test environments
  ...
  /**
   * Internal uninstall logic (without triggering external uninstall)
   */
  ...
  /**
   * Ensure listening for alarm events
   */
