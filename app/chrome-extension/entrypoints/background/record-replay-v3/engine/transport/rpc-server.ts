...
export class RpcServer {
   ...
   * @description Cancels a queued item, updates Run status, and publishes run.canceled event
   * @note Only allows canceling items with status=queued; running/paused items require rr_v3.cancelRun
   */
  private async handleCancelQueueItem(params: JsonObject | undefined): Promise<JsonValue> {
    ...
    // 2. Only allow canceling queued status (running/paused require rr_v3.cancelRun)
   ...
   * @description Saves or updates a Flow with full structural validation
   */
  private async handleSaveFlow(params: JsonObject | undefined): Promise<JsonValue> {
    ...
    // Check if updating an existing flow (query by trimmed ID)
    ...
    // Normalize flow, passing existingFlow to inherit createdAt
    const flow = this.normalizeFlowSpec(rawFlow, existingFlow);

    // Save to storage (storage layer performs secondary validation)
  ...
  private async handleDeleteFlow(params: JsonObject | undefined): Promise<JsonValue> {
    ...
    // Check for queued runs (unexecuted runs would fail after deletion)
