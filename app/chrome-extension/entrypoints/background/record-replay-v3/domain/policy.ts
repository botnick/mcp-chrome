 ...
 * @description Defines timeout, retry, error handling, and artifact policies for Record-Replay V3
 */
...
/**
 * Timeout policy
 * @description Defines timeout duration and scope for operations
 */
export interface TimeoutPolicy {
  /** Timeout in milliseconds */
  ms: UnixMillis;
  /** Scope: attempt=per attempt, node=entire node execution */
  scope?: 'attempt' | 'node';
}
...
export interface RetryPolicy {
  ...
  /** Backoff strategy: none=fixed interval, exp=exponential, linear=linear growth */
