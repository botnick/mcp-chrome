  ...
  return {
    ...
    async recoverOrphanLeases(
      ...
      return withTransaction(RR_V3_STORES.QUEUE, 'readwrite', async (stores) => {
         ...
         * @description
         * - Orphan: no lease or lease.ownerId !== currentOwnerId
         * - Reclaim: status -> queued, clear lease, preserve attempt
         */
        const recoverRunningItems = (): Promise<void> =>
          new Promise<void>((resolve, reject) => {
            ...
            request.onsuccess = () => {
              ...
              // Reclaim: remove lease, set status to queued
              const { lease: _droppedLease, ...itemWithoutLease } = item;
              const updated: RunQueueItem = {
                ...itemWithoutLease,
                status: 'queued',
        ...
        /**
         * Scan and adopt orphan paused items
         * @description
         * - Orphan: no lease or lease.ownerId !== currentOwnerId
         * - Adopt: keep status=paused, update lease.ownerId to new ownerId, renew TTL
         */
        const recoverPausedItems = (): Promise<void> =>
          new Promise<void>((resolve, reject) => {
            ...
            request.onsuccess = () => {
              ...
              // Adopt: update lease to new ownerId, renew TTL
              const updated: RunQueueItem = {
                ...item,
                updatedAt: now,
                lease: {
        ...
        // Execute sequentially: process running first, then paused
