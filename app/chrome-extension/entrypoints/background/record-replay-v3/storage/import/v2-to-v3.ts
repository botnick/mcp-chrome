 ...
 * @description Converts V2 format data to V3 format, supports bidirectional conversion
 */
 ...
 * @returns Conversion result with success/failure status, data, and error/warning messages
 */
...
/**
 * Convert a single V2 Node to V3 Node
 */
...
/**
 * Convert a single V2 Edge to V3 Edge
 */
 ...
 *
 * Rules:
 * 1. Exclude trigger-type nodes (UI-only, not part of execution)
 * 2. Only count edges between executable nodes for in-degree (ignore trigger outgoing edges)
 * 3. Find nodes with in-degree 0 as candidates
 * 4. If multiple candidates, use stable selection:
 *    - Prefer the top-left node by UI coordinates (x ascending, then y ascending)
 *    - If no UI coordinates, use lexicographic ID order
 */
function findEntryNodeId(nodes: NodeV3[], edges: EdgeV3[]): EntryNodeResult {
  const warnings: string[] = [];

  // 1. Exclude trigger nodes, get executable nodes
  ...
  // 2. Compute in-degree (only edges between executable nodes)
  const inDegree = new Map<NodeId, number>();
  for (const node of executableNodes) {
    inDegree.set(node.id, 0);
  }
  for (const edge of edges) {
    // Ignore edges from non-executable nodes (e.g., trigger)
  ...
  if (rootNodes.length === 0) {
    // No nodes with in-degree 0, graph may contain cycles, use stable selector for fallback
  ...
  // 5. Multiple root nodes, use stable selection rule
 ...
 * Prefer by UI coordinates (top-left first), then by lexicographic ID
 */
function selectStableRootNode(nodes: NodeV3[]): StableSelectionResult {
  // Check if node has valid UI coordinates
  ...
  if (nodesWithUi.length > 0) {
    // Sort by UI coordinates: x ASC -> y ASC -> id lexicographic (tie-breaker)
 ...
 * @param v2Trigger V2 format Trigger
 * @returns Conversion result
 */
...
function convertScheduleToCron(schedule: V2Trigger['schedule']): string | null {
  ...
  switch (schedule.type) {
    case 'interval': {
      // Convert interval to approximate cron (every N minutes)
    ...
    case 'daily':
      ...
      return '0 0 * * *'; // Default: daily at 0:00

    case 'weekly': {
      // Weekly on specified days and time
...
/**
 * Create a not-implemented V2ToV3Converter (backward compatible)
