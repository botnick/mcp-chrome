...
function createDisplayIcon(value: DisplayValue): SVGElement {
  ...
  // Container border (dashed rect representing the container)
  ...
  switch (value) {
    case 'block':
      // Two full-width block elements, vertically stacked
    ...
    case 'inline':
      ...
      break;
    case 'inline-block':
      // Block on the left, two lines of text on the right
...
function createHorizontalAlignIcon(value: AlignmentAxisValue): SVGElement {
  ...
  // Content block X coordinate varies by alignment mode
  ...
  // Two small blocks representing child elements (arranged vertically)
...
function createVerticalAlignIcon(value: AlignmentAxisValue): SVGElement {
  ...
  // Content block Y coordinate varies by alignment mode
  ...
  // Two small blocks representing child elements
  const block1 = document.createElementNS(SVG_NS, 'rect');
