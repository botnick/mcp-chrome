...
export function createEffectsControl(options: EffectsControlOptions): DesignControl {
  ...
  // Using WeakMap because:
  // 1. Hidden effects are not written to CSS (enabled=false), but need to be remembered within the session for restoration
  // 2. WeakMap automatically releases memory when the element is removed, no manual cleanup needed
  // 3. Only inline styles are read (not computed), so the cache only preserves the user's hide operations
  ...
  if (headerActionsContainer) {
    // Place the + button on the right side of the group header (left of chevron)
    headerActionsContainer.insertBefore(addBtn, headerActionsContainer.firstChild);
    disposer.add(() => addBtn.remove());
  } else {
    // Fallback: show toolbar in the content area
  ...
  function isEditing(): boolean {
    ...
    // Avoids overly broad focus detection preventing sync of external style changes
  ...
  function closePopover(opts?: { commit?: boolean; rollback?: boolean }): void {
    ...
    // Sync once after closing to ensure currentItems matches actual inline styles
    // Prevents inconsistencies from browser normalization/correction of values
    if (wasOpen && !rollback) {
      syncFromTarget(true);
    }
  }
  ...
  // Use document capture phase to ensure closing when clicking outside the Effects control
  ...
  // Listen for click events on document during capture phase
  ...
  // Listen for keyboard events on root during capture phase
