...
export class SemanticSimilarityEngine {
  ...
  private async _doInitializeWithProgress(
    ...
    try {
      ...
      // Prevent infinite loop: if already in offscreen document, force direct Worker mode
      ...
      if (this.useOffscreen) {
        // Use offscreen mode - delegate to offscreen document, which handles its own progress
        ...
        // Ensure config object is properly serialized with all properties explicitly set
        const configToSend = {
          modelIdentifier: this.config.modelIdentifier,
          localModelPathPrefix: this.config.localModelPathPrefix,
    ...
    try {
      ...
      if (this.useOffscreen) {
        ...
        const response = await chrome.runtime.sendMessage({
          target: 'offscreen',
          type: OFFSCREEN_MESSAGE_TYPES.SIMILARITY_ENGINE_INIT,
          config: serializedConfig, // Use original config, don't force-modify useLocalFiles
        });
        ...
        if (this.config.useLocalFiles) {
          // For WXT, resources under public/ are at root path at runtime
          // Use model identifier directly, transformers.js auto-adds /models/ prefix
        ...
        // For models that don't need token_type_ids, explicitly set in tokenizer config
  ...
  private async _initializeDirectWorkerWithProgress(
    ...
    try {
      ...
      // If progress callback not supported, fall back to standard loading
      console.log(
  ...
  public async warmupModel(): Promise<void> {
    ...
    // Representative warmup texts with different lengths and languages
    const warmupTexts = [
      // Short texts
      'Hello',
      'Hi there',
      ...
      'A simple sentence for warming up the model.',
      // Long texts
      'This is a longer text that contains multiple sentences. It helps warm up the model for various text lengths.',
      'Machine learning models benefit from warmup. This text helps initialize internal buffers for optimal performance.',
    ];
    ...
    try {
      ...
      // Keep warmup results, don't clear cache
  ...
  private async _tokenizeText(text: string | string[]): Promise<TokenizedOutput> {
    ...
    // For single text, try cache first
    ...
    // For batch text, process directly (batch processing rarely repeats)
    const startTime = performance.now();
    const tokenizerOptions: any = {
      padding: true,
      truncation: true,
  ...
  private _extractEmbeddingFromWorkerOutput(
    ...
    // Optimization: use Float32Array directly, avoid unnecessary conversion
    ...
    // Get embedding array from memory pool
  ...
  public async getEmbeddingsBatch(
    ...
    if (this.useOffscreen) {
      ...
      // If all are cached, return directly
      if (uncachedTexts.length === 0) {
        return results as Float32Array[];
      }

      // Only request uncached texts
      const response = await chrome.runtime.sendMessage({
        target: 'offscreen',
        type: OFFSCREEN_MESSAGE_TYPES.SIMILARITY_ENGINE_BATCH_COMPUTE,
        texts: uncachedTexts,
      ...
      // Place results back in corresponding positions and cache
  ...
  public cosineSimilarity(vecA: Float32Array, vecB: Float32Array): number {
    ...
    if (this.useSIMD && this.simdMath) {
      try {
        // SIMD version is async, but we need sync for interface compatibility
        // Falling back to JavaScript version; consider refactoring to async
